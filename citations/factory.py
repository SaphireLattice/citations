...
class Factory:
    ...
    def _check_options(self, _skip_=False, lines=None, penalty=None, title=None,
    ...
    def _generate(self, draw: ImageDraw):
        ...
        for line in range(len(self.lines)):
            self._text_line(draw, line + offset, self.lines[line])
    ...
    def _print_barcode(self, draw: ImageDraw, spec: List[int] = None, color: str = None):
        if spec is None:
            spec = self.barcode
        if color is None:
            color = self.color_fg
